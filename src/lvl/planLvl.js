...
function planLvl(jobFiles, parsedStock = null) {
  ...
  const stockByDepth = new Map();    // depth -> on-hand lf
  const incomingByDepth = new Map(); // depth -> on-PO lf (shown, never netted)
  if (hasStock) {
    for (const item of parsedStock.items) {
      ...
      if (item.incoming) {
        incomingByDepth.set(depth, (incomingByDepth.get(depth) || 0) + item.incoming * item.span);
      }
    }
  ...
  const byDepth = [...allDepths].map((depth) => {
    ...
    const incomingLf = hasStock ? round2(incomingByDepth.get(depth) || 0) : null;
    return {
      depth,
