...
//
// `incoming` is SURFACED but NEVER SUBTRACTED: it is carried onto each item so
// tabs can show it as an informational column (an inbound PO may not land before
// the job ships, so it must not net against demand), exactly as the Plates and
// Hangers readers do. The solver's `qty` baseline still excludes it.
//
// IGNORED on purpose: `on_hand`/`committed` (superseded by `available`), `flag`
// (a REORDER flag computed against TODAY's stock — the useful signal is what
// drops below threshold AFTER this batch ships, which inventoryImpact computes
// itself), and `depth` (see below).
// =============================================================
...
function parseStockCsv(text) {
  ...
  for (let r = headerRow + 1; r < rows.length; r++) {
    ...
    // Informational only — surfaced for display, never netted against demand.
    // Strip thousands separators first ("1,200" → 1200), matching the hanger and
    // plate readers; a bare Number("1,200") is NaN and would silently drop it.
    if (iIncoming !== -1) {
      const inc = Number(String(cols[iIncoming] ?? '').replace(/,/g, '').trim());
      if (Number.isFinite(inc) && inc > 0) inv.incoming = inc;
    }
