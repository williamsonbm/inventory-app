...
- **Read an issue**: `gh api repos/<owner>/<repo>/issues/<number> --jq '.body'` for the body, and
  ...
  comments. **`gh issue view <number> --comments` does not work in this repo** — it fails with a
  GraphQL "Projects (classic) is being deprecated" error and returns nothing. An agent that
  trusts it concludes an issue has no comments, and every decision in this repo lives in a
  comment. `gh issue view <number> --json body,labels,title --jq ...` does work; only the
  `--comments` flag is affected.
